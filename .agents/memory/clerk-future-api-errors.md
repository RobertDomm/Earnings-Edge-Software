...
All Clerk v6 Future API methods (`signIn.emailCode.sendCode/verifyCode`, `signIn.finalize`, `signUp.create`, `signUp.verifications.sendEmailCode/verifyEmailCode`, `signUp.finalize`, etc.) resolve to `{ error: ClerkError | null }` and **never throw** on API errors. `error.code` is the machine-stable code (e.g. `form_identifier_not_found`); `error.longMessage` is user-displayable.
